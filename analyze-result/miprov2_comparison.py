...
def create_miprov2_heatmap(csv_paths, model_names=None, output_dir='miprov2_analysis'):
    """
    Create a benchmark model heatmap specifically for MIPROv2 optimizer.
    ...
    # Filter for MIPROv2 optimizer
    miprov2_df = combined_df[combined_df['optimizer'] == 'MIPROv2']
    
    # Print basic information
    print(f"Analyzing MIPROv2 optimizer across {len(model_names)} models")
    print(f"Models: {', '.join(model_names)}")
    print(f"Total number of benchmark runs with MIPROv2: {len(miprov2_df)}")
    print(f"Unique benchmarks: {miprov2_df['benchmark'].nunique()}")
    
    # Set the style for all plots
    sns.set(style="whitegrid")
    plt.rcParams.update({'font.size': 10})
    
    # Create benchmark model heatmap for MIPROv2
    plt.figure(figsize=(16, 12))
    
    # Create pivot table for benchmark vs model with MIPROv2
    benchmark_model_pivot = miprov2_df.pivot_table(
        index='benchmark', 
        columns='model', 
        values='score', 
        aggfunc='mean'
    ...
    plt.title('Benchmark Performance by Model with MIPROv2 Optimizer', fontsize=16)
    plt.xlabel('Model', fontsize=12)
    plt.ylabel('Benchmark', fontsize=12)
    plt.tight_layout()
    
    # Save the heatmap
    heatmap_path = output_path / 'miprov2_benchmark_model_heatmap.png'
    ...
    if 'Baseline' in combined_df['optimizer'].unique():
        ...
        for model in model_names:
            for benchmark in miprov2_df['benchmark'].unique():
                ...
                # Get MIPROv2 score for this model and benchmark
                mipro_score = miprov2_df[(miprov2_df['model'] == model) & 
                                              (miprov2_df['benchmark'] == benchmark)]['score'].mean()
        ...
        plt.title('MIPROv2 Improvement Over Baseline (%)', fontsize=16)
        plt.xlabel('Model', fontsize=12)
        plt.ylabel('Benchmark', fontsize=12)
        plt.tight_layout()
        
        # Save the improvement heatmap
        improvement_path = output_path / 'miprov2_improvement_heatmap.png'
...
if __name__ == "__main__":
    # Compare multiple models with MIPROv2 optimizer
    csv_paths = [
    ...
    miprov2_df = create_miprov2_heatmap(csv_paths, model_names)
